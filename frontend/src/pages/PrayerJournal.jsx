...
import {
  Plus, Trash2, CheckCheck, RotateCcw, Calendar, X,
  Flame, Star, Search, ChevronDown, ChevronUp, Heart,
  Home, Users, BookOpen, Globe, Filter, Pencil,
} from 'lucide-react';
...
/* ── Categories ──────────────────────────────────────────────────────────── */
const CATEGORIES = [
  { id: 'all',       label: 'All',       icon: Filter,   color: 'bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400'       },
  { id: 'personal',  label: 'Personal',  icon: Heart,    color: 'bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400'         },
  { id: 'family',    label: 'Family',    icon: Home,     color: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400'     },
  { id: 'timothy',   label: 'Timothys',  icon: Users,    color: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400'         },
  { id: 'church',    label: 'Church',    icon: BookOpen, color: 'bg-forest-100 dark:bg-forest-900/30 text-forest-700 dark:text-forest-400' },
  { id: 'community', label: 'Community', icon: Globe,    color: 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400' },
];
...
const RequestDialog = ({ open, onClose, onSave, editData }) => {
  ...
  const handleSave = () => {
    if (!text.trim() && !person.trim()) {
      toast.error('Please enter a person or prayer request.');
      return;
    }
    onSave({
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-stone-500 dark:text-stone-400">
            Bring it before the Lord with faith
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-2">
          ...
          <div>
            ...
            <Input value={person} onChange={e => setPerson(e.target.value)}
              placeholder="e.g. Juan dela Cruz, our church plant…"
              className={ic} />
          </div>
          ...
          <div>
            ...
            <textarea value={text} onChange={e => setText(e.target.value)}
              placeholder="Write your prayer request… (or fill specific items below)"
              rows={3}
              className="w-full px-3 py-2 rounded-lg border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-stone-700 text-sm text-stone-900 dark:text-stone-100 outline-none focus:border-forest-400 transition-colors placeholder:text-stone-400 resize-none" />
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {CATEGORIES.filter(c => c.id !== 'all').map(c => (
                <button key={c.id}
                  onClick={() => { setCategory(c.id); setItems({}); }}
          ...
          {/* Specific prayer items (collapsible) */}
          {templates.length > 0 && (
            <div>
              ...
              {showItems && (
                <div className="space-y-3 rounded-xl p-4 border border-stone-100 dark:border-stone-700 bg-stone-50 dark:bg-stone-900/30">
                  <p className="text-xs text-stone-400 dark:text-stone-500 -mt-1 mb-1">
                    Fill in specific requests per area:
                  </p>
                  {templates.map(t => (
                    <div key={t.key}>
                      <Label className="text-xs font-semibold text-forest-700 dark:text-forest-400 mb-1 block">
                        {t.label}
                      </Label>
                      <textarea
                        value={items[t.key] || ''}
                        ...
                        rows={2}
                        className="w-full px-3 py-2 rounded-lg border border-stone-200 dark:border-stone-600 bg-white dark:bg-stone-700 text-sm text-stone-700 dark:text-stone-300 outline-none focus:border-forest-400 transition-colors placeholder:text-stone-300 dark:placeholder:text-stone-600 resize-none"
                      />
                    </div>
                  ))}
                </div>
...
const MarkAnsweredDialog = ({ open, request, onClose, onSave }) => {
  const [dateAnswered, setDateAnswered] = useState(todayIso());
  const [praise,       setPraise]       = useState('');

  useEffect(() => {
    if (open) { setDateAnswered(todayIso()); setPraise(''); }
  }, [open]);
        ...
        <div className="space-y-4 mt-2">
          ...
          <div>
            ...
            <textarea value={praise} onChange={e => setPraise(e.target.value)}
              ...
              className="w-full px-3 py-2 rounded-lg border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-stone-700 text-sm text-stone-900 dark:text-stone-100 outline-none focus:border-forest-400 transition-colors placeholder:text-stone-400 resize-none font-serif" />
          </div>
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            {req.person && (
              <p className="text-xs font-semibold text-stone-500 dark:text-stone-400 uppercase tracking-wide mb-0.5">
                {req.person}
              </p>
            )}
            {req.text && (
...
const PrayerJournal = () => {
  ...
  const [activeView,   setActiveView]   = useState('ongoing'); // 'ongoing' | 'praises'
  ...
  const applyFilters = (list) => list
    ...
    .filter(r => {
      ...
      if (!q) return true;
      return [r.person, r.text, ...Object.values(r.items || {})].join(' ').toLowerCase().includes(q);
    });
          ...
          {filteredOngoing.length > 0 ? (
            filteredOngoing.map(req => (
              <RequestCard key={req.id} req={req}
                onMarkAnswered={setMarkTarget} onDelete={handleDelete}
                onUnmark={handleUnmark}       onEdit={setEditTarget} />
            ))
          ) : (
        ...
        <Card className="bg-white dark:bg-stone-800 rounded-xl shadow-sm border border-stone-100 dark:border-stone-700 overflow-hidden">
          <button onClick={() => setShowAnswered(v => !v)}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-stone-400 dark:text-stone-500">({filteredAnswered.length})</span>
            </div>
      ...
      {answered.length === 0 ? (
        <Card className="bg-white dark:bg-stone-800 rounded-xl shadow-sm border border-stone-100 dark:border-stone-700">
          <div className="text-center py-14 text-stone-400 dark:text-stone-500">
            ...
            <p className="text-xs mt-1 max-w-xs mx-auto text-stone-400 dark:text-stone-500 leading-relaxed">
        ...
        <>
          ...
          {answered.map(req => {
            ...
            const catLabel = CATEGORIES.find(c => c.id === req.category)?.label || req.category;
            return (
              <Card key={req.id} className="bg-white dark:bg-stone-800 rounded-xl shadow-sm border border-stone-100 dark:border-stone-700 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2.5">
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${cat.color}`}>{catLabel}</span>
                      <span className="text-[10px] text-stone-400 dark:text-stone-500 flex items-center gap-1">
  ...
  const viewToggle = (
    <div className="flex gap-1 bg-stone-100 dark:bg-stone-800 rounded-xl p-1">
      <button onClick={() => setActiveView('ongoing')}
      ...
      <button onClick={() => setActiveView('praises')}
        className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all ${
          activeView === 'praises'
  ...
  const headerContent = (
    <div className="relative overflow-hidden rounded-2xl text-white"
      style={{
        background: 'linear-gradient(160deg, #1c1917 0%, #4c1d95 40%, #be123c 100%)',
      ...
      <div className="relative z-10">
        ...
        <h1 className={`font-serif font-bold tracking-tight mb-1 ${isTablet ? 'text-4xl' : 'text-3xl'}`}>
          Prayer Journal
        </h1>
      ...
      {isTablet ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6 items-start">
            <div className="space-y-4">
              {viewToggle}
              {activeView === 'ongoing' && ongoingView}
              {activeView === 'praises' && praisesView}
            </div>
            <div className="space-y-4">
                  ...
                  {[
                    ...
                    { label: 'Answer Rate',
                      value: requests.length
                        ? `${Math.round((answered.length / requests.length) * 100)}%`
                        : '—' },
                  ].map(s => (
              ...
              <Card className="bg-forest-50 dark:bg-forest-900/20 rounded-xl border border-forest-100 dark:border-forest-800 p-5">
                <p className="text-xs font-bold uppercase tracking-widest text-forest-700 dark:text-forest-400 mb-2">
