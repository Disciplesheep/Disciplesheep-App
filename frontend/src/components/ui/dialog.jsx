...
const DialogContent = React.forwardRef(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        // Position: horizontally centered, vertically in upper-third (not dead-center)
        "fixed left-[50%] top-[10%] z-50 grid w-[calc(100vw-2rem)] max-w-lg translate-x-[-50%]",
        // No translate-y — top-[10%] places it near the top with equal side margins
        "max-h-[82vh] overflow-hidden",
        // Appearance
        "gap-4 border bg-background p-6 shadow-lg",
        // Match card roundness (rounded-xl = 0.75rem, but var(--radius) = 1rem which is even rounder)
        "rounded-xl",
        // Animations — updated slide-in origin to match new top position
        "duration-200",
