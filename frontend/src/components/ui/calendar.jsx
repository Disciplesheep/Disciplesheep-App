    ...
    <div style={{ position: 'relative', width: '100%' }}>
      <div style={{
        ...
        <button
          ...
          style={{ fontSize: '0.875rem', fontWeight: 600, fontFamily: 'inherit', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 8px', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 4, color: 'inherit', minHeight: 0, transition: 'background 0.15s' }}
          onMouseEnter={e => e.currentTarget.style.background = 'hsl(var(--accent))'}
          onMouseLeave={e => e.currentTarget.style.background = 'none'}
          title="Click to pick a year"
        >
          {label}
          <span style={{ fontSize: '0.6rem', opacity: 0.5, transform: showYearPicker ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s', display: 'inline-block' }}>▼</span>
        </button>
        {!showYearPicker && (
          <button
            onClick={nextMonth}
      ...
      {showYearPicker && (
        <YearPicker displayMonth={displayMonth} fromDate={fromDate} toDate={toDate} onSelect={handleYearSelect} />
      )}
    </div>
  );
};
